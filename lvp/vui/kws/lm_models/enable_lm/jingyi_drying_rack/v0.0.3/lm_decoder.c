                     ...
                     float              *bm_activation_score)
{
    ...
    if (ctc_score > 85.f && ctc_score > g_kws_list.kws_param_list[ctc_kws_index].threshold/10.f + 5.f) {
        ...
        if (0) {
            ...
            if (ctc_score > g_kws_list.kws_param_list[ctc_kws_index].threshold/10.f + 10.f) {
#ifdef __OPEN_PRINTF__
                printf ("#### %s Do Not need BM:%f\n", g_kws_list.kws_param_list[ctc_kws_index].kws_words, ctc_score);
#endif
                return 0;
            }
        }
