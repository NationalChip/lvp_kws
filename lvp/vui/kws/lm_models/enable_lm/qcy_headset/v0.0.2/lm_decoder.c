                     ...
                     float              *bm_activation_score)
{
    ...
    if ((g_kws_list.kws_param_list[ctc_kws_index].major&0xf0) != 0x80 || ctc_score > g_kws_list.kws_param_list[ctc_kws_index].threshold/10.f + 4.f) {
        return 0;
    }
#else
    if ((g_kws_list.kws_param_list[ctc_kws_index].major&0xf0) != 0x80 || ctc_score > g_kws_list.kws_param_list[ctc_kws_index].threshold/10.f + 5.9f) {
        return 0;
    }
#endif
    ...
    for (int i = 0; i < CONFIG_BEAM_SIZE; i++) {
        for (int j = 0; j < g_kws_list.count; j++) {
            ...
            //printf ("#@#@ [%s, %s] leven_dist:%d, dimsim:%f\n", g_kws_list.kws_param_list[j].kws_words, bm_str[i], _GetKwsLevenshtein(g_kws_list, j, bm_str[i]), dimsim_dist);
