...
float _GetKwsDimSim(LVP_KWS_PARAM_LIST g_kws_list, int kws_index, int beam_index)
{
    ...
    if (g_kws_list.kws_param_list[kws_index].label_length > CONFIG_PREFIX_CHAR_LENGTH) {
        len = CONFIG_PREFIX_CHAR_LENGTH;
        ctc_offset = g_kws_list.kws_param_list[kws_index].label_length - CONFIG_PREFIX_CHAR_LENGTH;
    }
    else {
        len = g_kws_list.kws_param_list[kws_index].label_length;
        bm_offset = CONFIG_PREFIX_CHAR_LENGTH - g_kws_list.kws_param_list[kws_index].label_length;
    }

    for (int m = 0, n = 0; m < len; ) {
        unsigned int a_id = kws[n + ctc_offset];
        unsigned int b_id = prefix->prefix_set_prev[m + bm_offset] - 1;
        if (b_id == 64 || b_id > 64) {// 跳过开头、结尾的blank音节
            m ++;
            continue;
        }
...
#if defined (CONFIG_LVP_KWS_XIAO_DU_XIAO_DU_V0DOT1DOT9_2021_0426)
    if ((g_kws_list.kws_param_list[ctc_kws_index].major&0xf0) != 0x80 || ctc_score > g_kws_list.kws_param_list[ctc_kws_index].threshold/10.f + 4.0f) {
        return 0;
    }
#elif defined (CONFIG_LVP_KWS_GRUS_TWS_ZH_V0DOT2DOT0_2021_0527)
    if ((g_kws_list.kws_param_list[ctc_kws_index].major&0xf0) != 0x80 || ctc_score > g_kws_list.kws_param_list[ctc_kws_index].threshold/10.f + 4.0f) {
        return 0;
    }
#else
    if ((g_kws_list.kws_param_list[ctc_kws_index].major&0xf0) != 0x80 || ctc_score > g_kws_list.kws_param_list[ctc_kws_index].threshold/10.f + 3.0f) {
    return 0;
    }
#endif
        ...
        if (p >= win_length - 1) {
            ...
            for (int i = 0; i < CONFIG_BEAM_SIZE; i++) {
                ...
                for (int j = 0; j < g_kws_list.count; j++) {
                    if (prefix_length[i] < g_kws_list.kws_param_list[j].label_length) continue;
...
#endif
                    } else if (dimsim_dist < mini_dimsim_dist && dimsim_dist < 100.f) {
