...
#include "ctc_model.h"
#include "alexa_kws_model_v0.1.0_2020_0427.h"

#define ALIGNED_16(x)   ((x+15)/16*16)
#define LOG_TAG "[KWS]"

const char *kws_version = "alexa_kws_model_v0.1.0_2020_0427.h";
...
int LvpCTCModelInitSnpuTask(GX_SNPU_TASK *snpu_task)
{
    ...
    if (sizeof(struct kws_in_out) != ALIGNED_16(sizeof(struct input) + sizeof(struct output))) {
        printf ("Pls checkout kws_in_out in ctc_model.c, [%d, %d]\n", sizeof(struct kws_in_out), ALIGNED_16(sizeof(struct input) + sizeof(struct output)));
        return -1;
    }
