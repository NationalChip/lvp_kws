...
static float last_gain[CONFIG_DENOISE_MODEL_OUTPUT_LENGTH];
...
static void _NnDenoiseGetHardwareFft(LVP_CONTEXT *context)
{
    ...
    for(int i = 0; i < LvpGetPcmFrameNumPerContext(); i++) {
        ...
        if ((s_fft_pack_count % CONFIG_DENOISE_MODEL_INPUT_STRIDE_LENGTH == 0)) {
		...
		else if (last_frame_out_snpu) {
			last_frame_out_snpu = 0;
			if (s_denoise_trans_flag) {
				printf("2:%d\n", out_ctx->ctx_index);
                RecoverDenoiseWav(out_ctx,
			...
			APP_EVENT event = {
            	...
            	.ctx_index = &wav_buffer[(start+1) * NN_DENOISE_WAV_LENGTH],
        	};
        	LvpTriggerAppEvent(&event);
#ifdef CONFIG_LVP_HAS_UART_RECORD
       	 	UartRecordChannelTask((unsigned char *)&wav_buffer[(start+1) * NN_DENOISE_WAV_LENGTH],
...
static void _RecoverTick(void)
{
    ...
    if (LvpQueueGet(&s_denoise_task_queue, (unsigned char *)&module_info)) {
        ...
        start = wav_offset %= CONFIG_NN_DENOISE_FFT_TMP_BUFFER_DEEPTH;
        for (int i = 0; i < 1; i++) {
            if (s_denoise_trans_flag) {
                int idx = ((LVP_CONTEXT *)(module_info.priv))->ctx_index;
                printf("1:%d \n", idx);
