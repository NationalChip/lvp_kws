...
static float hamming_buff[64] ALIGNED_ATTR(16) =
    {0.080000, 0.082215, 0.088839, 0.099807, 0.115015, 0.134316, 0.157524, 0.184415, \
     ...
     1.000000, 0.997785, 0.991161, 0.980193, 0.964985, 0.945684, 0.922476, 0.895585, \
...
static float s_win_buffer[3][G_VAD_FFT_LEN] ALIGNED_ATTR(16) DRAM0_AUDIO_IN_ATTR = {0}; // for one window G-sensor data to float , solo channel
...
static S_VOICEPRINT_STATE s_voiceprint_state[G_SENSOR_SAMPLE_DIM] ALIGNED_ATTR(16) ;
...
#endif
//        printf("%f\n", m1 + m2 + m3 + m4);
        vp_state->cur_max_frequency_index = m1 < 0.02f ? 255 : index_max; // 最高能频带能量低于一定值，认为是底噪，覆盖为无效值
        s_G_vad_handle.feature_sum[G_stride_index] += m1 + m2 + m3 + m4;
...
//            ttt = gx_get_time_us();
        ...
        vp_state->counter_p = (vp_state->counter_p + 1) >= CONFIG_LVP_G_SENSOR_VAD_VOICE_COUNT_WINDOW \
                              ? ((vp_state->counter_p + 1) - CONFIG_LVP_G_SENSOR_VAD_VOICE_COUNT_WINDOW) \
...
{
//            unsigned int ttt = gx_get_time_us();
    memset(&s_G_vad_handle.feature_sum[0], 0, 10 * 4);
...
//    printf("o %d\n", ignore_dim);
//    printf("n %d\n", s_G_vad_handle.fft_frame_offset_next);


//            printf("d3 %d\n", gx_get_time_us() - ttt);
//            ttt = gx_get_time_us();
    unsigned int stride_num = G_size / (G_SENSOR_SAMPLE_SIZE * s_G_vad_handle.config.fft_frame_shift);
    unsigned int w_len = s_G_vad_handle.config.G_vad_voice_count_window;
    unsigned int map_index = ((s_G_vad_handle.config.G_vad_voice_count_window + s_voiceprint_state[0].counter_p + 1 - stride_num)
                                > s_G_vad_handle.config.G_vad_voice_count_window) ?
                             ((s_G_vad_handle.config.G_vad_voice_count_window + s_voiceprint_state[0].counter_p + 1 - stride_num)
                                - s_G_vad_handle.config.G_vad_voice_count_window) :
                             (s_G_vad_handle.config.G_vad_voice_count_window + s_voiceprint_state[0].counter_p + 1 - stride_num);
    for (int i = 0; i < stride_num; i++) {
#if G_VAD_LOG
            printf("%f/%f\n", s_G_vad_handle.feature_sum[i] , s_G_vad_handle.config.G_vad_threshold);
#endif
            s_G_vad_handle.feature_state[(map_index + i >= w_len) ? ((map_index + i) - w_len) : (map_index + i) ] \
                = s_G_vad_handle.feature_sum[i] > s_G_vad_handle.config.G_vad_threshold;
    }
#if 0
    for (int k = 0; k < s_G_vad_handle.config.G_vad_voice_count_window; k++) {
        printf("%d ", s_G_vad_handle.feature_state[k]);
    }
           printf("\n");
#endif
...
//           printf("\n");
//        printf("%d ", s_G_vad_handle.feature_state[k]);
        feature_state_counter += s_G_vad_handle.feature_state[k];
    }
//            printf("\n");
    printf("f_counter %d\n", feature_state_counter);
