...
*
* This file comes from csky_dct4_q15.c. It is renamed by replacing arm with csky.
*
*/

/******************************************************************************
 * @file     csky_dct4_q15.c
...
__ALWAYS_STATIC_INLINE void rfft_result_copy_q15(q15_t *pSrc, uint16_t length)
{
  uint32_t i;
  q15_t *pS1, *pS2;
 ...
 * \code
 * csky_dct4_q15(&csky_dct4_sR_q15_len128, pState, pSrc);
 * csky_dct4_q15(&csky_dct4_sR_q15_len512, pState, pSrc);
 * csky_dct4_q15(&csky_dct4_sR_q15_len2048, pState, pSrc);
 * csky_dct4_q15(&csky_dct4_sR_q15_len8192, pState, pSrc);
 * \endcode
 *
 * \par Input an output formats:
 * Input samples need to be downscaled by 1 bit to avoid saturations in the Q15 DCT process,
  ...
  q15_t * pInlineBuffer)
{
  ...
  csky_mult_q15(pInlineBuffer, cosFact, pInlineBuffer, S->N);
  csky_shift_q15(pInlineBuffer, 1, pInlineBuffer, S->N);
  ...
  csky_rfft_q15(S->pRfft, pInlineBuffer, pState);

  /* Add the conjugate part for DCT usage.*/
  rfft_result_copy_q15(pState, S->N);

 /*----------------------------------------------------------------------
  *  Step3: Multiply the FFT output with the weights.
  *----------------------------------------------------------------------*/
  csky_cmplx_mult_cmplx_q15(pState, weights, pState, S->N);
  ...
  csky_shift_q15(pState, 2, pState, S->N * 2);
