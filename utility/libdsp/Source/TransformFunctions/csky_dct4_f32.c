...
*
* This file comes from arm_dct4_f32.c. It is renamed by replacing arm with csky.
*
*/

/******************************************************************************
 * @file     csky_dct4_f32.c
...
__ALWAYS_STATIC_INLINE void rfft_result_copy_f32(float32_t *pSrc, uint16_t length)
{
  uint32_t i;
  float32_t *pS1, *pS2;
 ...
 *  As DCT4 internally uses Real FFT, it supports all the lengths supported by csky_rfft_fast_f32().
 ...
 * - Initializes Real FFT as its process function is used internally in DCT4, by calling csky_rfft_fast_init_f32().
 * \par
 ...
 * The CFFT and RFFT structures also needs to be initialized, refer to csky_cfft_f32()
 * and csky_rfft_fast_f32() respectively for details regarding static initialization.
 *
 ...
 * \code
 * csky_dct4_f32(&csky_dct4_sR_f32_len128, pState, pSrc);
 * csky_dct4_f32(&csky_dct4_sR_f32_len512, pState, pSrc);
 * csky_dct4_f32(&csky_dct4_sR_f32_len2048, pState, pSrc);
 * csky_dct4_f32(&csky_dct4_sR_f32_len8192, pState, pSrc);
 * \endcode
 */
  ...
  float32_t * pInlineBuffer)
{
  ...
  csky_mult_f32(pInlineBuffer, cosFact, pInlineBuffer, S->N);
  ...
  csky_rfft_fast_f32(S->pRfft, pInlineBuffer, pState, 0);
   ...
   */
  csky_cmplx_mult_cmplx_f32(pState, weights, pState, S->Nby2+1);

  csky_cmplx_mult_cmplx_re_f32(pState+S->N+2, &weights[S->N], pState+S->N+2, S->Nby2-1);
  ...
  *  Step3: Multiply the FFT output with the weights.
  */
  csky_cmplx_mult_cmplx_f32(pState, weights, pState, S->N);
