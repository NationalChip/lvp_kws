...
*
* This file comes from arm_dct4_q31.c. It is renamed by replacing arm with csky.
*
*/

/******************************************************************************
 * @file     csky_dct4_q31.c
 * @brief    Processing function of DCT4 & IDCT4 Q31.
 * @version  V1.0
 * @date     20. Dec 2016
 ******************************************************************************/
...
__ALWAYS_STATIC_INLINE void rfft_result_copy_q31(q31_t *pSrc, uint16_t length)
{
  uint32_t i;
  q31_t *pS1, *pS2;
 ...
 * \code
 * csky_dct4_q31(&csky_dct4_sR_q31_len128, pState, pSrc);
 * csky_dct4_q31(&csky_dct4_sR_q31_len512, pState, pSrc);
 * csky_dct4_q31(&csky_dct4_sR_q31_len2048, pState, pSrc);
 * csky_dct4_q31(&csky_dct4_sR_q31_len8192, pState, pSrc);
 * \endcode
 *
  ...
  q31_t * pInlineBuffer)
{
  ...
  csky_mult_q31(pInlineBuffer, cosFact, pInlineBuffer, S->N);
  csky_shift_q31(pInlineBuffer, 1, pInlineBuffer, S->N);
  ...
  csky_rfft_q31(S->pRfft, pInlineBuffer, pState);

  /* Add the conjugate part for DCT usage.*/
  rfft_result_copy_q31(pState, S->N);
  
  /*----------------------------------------------------------------------
   *  Step3: Multiply the FFT output with the weights.
   *----------------------------------------------------------------------*/
  csky_cmplx_mult_cmplx_q31(pState, weights, pState, S->N);
  ...
  csky_shift_q31(pState, 2, pState, S->N * 2);
