...
*
* This file comes from arm_conv_partial_fast_opt_q15.c. It is renamed by replacing arm with csky.
*
*/

/******************************************************************************
 * @file     csky_conv_partial_fast_opt_q15.c
 * @brief    Fast Q15 Partial convolution.
 * @version  V1.0
 * @date     20. Dec 2016
 ******************************************************************************/
 ...
 *
 * See <code>csky_conv_partial_q15()</code> for a slower implementation of this function which uses a 64-bit accumulator to avoid wrap around distortion.
 *
 * \par Restrictions
...
{/*{{{*/
  ...
  else
  {
    ...
    csky_copy_q15(pIn1, pScr1, srcALen);
  ...
  q15_t * pScratch2)
{
  ...
  q15_t *py;                                     /* Intermediate inputB pointer  */
  uint32_t j, k, blkCnt;                         /* loop counter */
  csky_status status;                             /* Status variable */
  uint32_t tapCnt;                               /* loop count */
  ...
  else
  {
    ...
    {/*{{{*/
      ...
      /* Store the results in the accumulators in the destination buffer. */
      *pOut++ = __SSAT_16((acc0 >> 15));
      *pOut++ = __SSAT_16((acc1 >> 15));
      *pOut++ = __SSAT_16((acc2 >> 15));
      *pOut++ = __SSAT_16((acc3 >> 15));
