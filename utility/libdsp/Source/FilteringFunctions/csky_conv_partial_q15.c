...
*
* This file comes from arm_conv_partial_q15.c. It is renamed by replacing arm with csky.
*
*/

/******************************************************************************
 * @file     csky_conv_partial_q15.c
 ...
 *
 * \par
 * Refer the function <code>csky_conv_partial_opt_q15()</code> for a faster implementation of this function using scratch buffers.
 *
 */
...
csky_status csky_conv_partial_q15(
  ...
  uint32_t numPoints)
{
#if (!defined(CSKY_MATH_NO_SIMD)) && !defined(UNALIGNED_SUPPORT_DISABLE)
  ...
  csky_status status;                             /* status of Partial convolution */
  ...
  if(srcBLen >= 4u)
  {
    ...
    while(blkCnt > 0u)
    {
...
#endif /*      #ifdef  CSKY_MATH_BIG_ENDIAN     */
      ...
      *__SIMD32(pOut)++ =
        __PKHBT(__SSAT_16((acc0 >> 15)), __SSAT_16((acc1 >> 15)), 16);
      *__SIMD32(pOut)++ =
        __PKHBT(__SSAT_16((acc2 >> 15)), __SSAT_16((acc3 >> 15)), 16);

#else

      *__SIMD32(pOut)++ =
        __PKHBT(__SSAT_16((acc1 >> 15)), __SSAT_16((acc0 >> 15)), 16);
      *__SIMD32(pOut)++ =
        __PKHBT(__SSAT_16((acc3 >> 15)), __SSAT_16((acc2 >> 15)), 16);
  ...
  if((firstIndex + numPoints) > ((srcALen + (srcBLen - 1u))))
  {
    /* Set status as CSKY_ARGUMENT_ERROR */
    status = CSKY_MATH_ARGUMENT_ERROR;
  }
  else
  {
    ...
    for (i = firstIndex; i <= (firstIndex + numPoints - 1); i++)
    {
      ...
      pDst[i] = (q15_t) __SSAT_16((sum >> 15u));
    }
    /* set status as CSKY_SUCCESS as there are no argument errors */
    status = CSKY_MATH_SUCCESS;
  }
  return (status);
