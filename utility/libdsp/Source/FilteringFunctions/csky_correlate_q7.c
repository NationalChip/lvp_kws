...
*
* This file comes from arm_correlate_q7.c. It is renamed by replacing arm with csky.
*
*/

/******************************************************************************
 * @file     csky_correlate_q7.c
 ...
 *
 * \par
 * Refer the function <code>csky_correlate_opt_q7()</code> for a faster implementation of this function.
 *
 */
  ...
  q7_t * pDst)
{
  ...
  for (i = 0u; i <= tot; i++)
  {
    ...
    for (j = 0u; j <= i; j++)
    {
      ...
      {
        /* z[i] += x[i-j] * y[j] */
        sum += ((q15_t) pIn1[j] * pIn2[(int32_t)(-((int32_t) i - j))]);
      }
    }
    /* Store the output in the destination buffer */
    if(inv == 1)
      *pDst-- = (q7_t) __SSAT_8((sum >> 7u));
    else
      *pDst++ = (q7_t) __SSAT_8((sum >> 7u));
  }
