...
*
* This file comes from arm_correlate_f32.c. It is renamed by replacing arm with csky.
*
*/

/******************************************************************************
 * @file     csky_correlate_f32.c
 ...
 * The CSI library provides correlation functions for Q7, Q15, Q31 and floating-point data types.
  ...
  float32_t * pDst)
{
  ...
  for (i = 0u; i <= tot; i++)
  {
    ...
    for (j = 0u; j <= i; j++)
    {
      ...
      {
        /* z[i] += x[i-j] * y[j] */
        sum += pIn1[j] * pIn2[(int32_t)(-((int32_t) i - j))];
      }
    }
