...
/**
 * @defgroup ExponFun Exponential series Functions
 *
 * Exponentional series functions includes pow, pow2 and exp, which are
 * represented any value based, 2 based and e based Exponentional function
 * repectively. The common methods to compute these functions are ploynomial
 * approximation and table loop-up. And, it is a double floating version for the
 * functions, say, the input and output are both double.
 *
 */
 ...
 * @param[in] x input value in double, the base of pow.
 * @param[in] x input value in double, the exponention of pow.
 * @return x^y.
 *
 * The implementation is based on the transform <pre> x^y = e^(y*log(x))</pre>
 *
...
{
  float64_t z,a,aa,error, t,a1,a2,y1,y2;
  mynumber u,v;
  q31_t k;
  q31_t qx,qy;
  ...
  { /* if y<-1 or y>1 */
      ...
      a2     = (a - a1) + aa;
      error  = error * ABS(y);
      retval = _exp1(a1, a2, 1.9e16*error);
      return retval;
  }
...
/**
 * Fast approximation to the logarithm function for float64_t-point data.
...
/**
 * Fast approximation to the exponential function for float64_t-point data.
 * param[in]  x  input value in double.
...
/**
 * A function to check the double value is intger, odd, even or neither.
 * param[in]  x  input value in double.
 * return flag.
 */
static q31_t _checkint(float64_t x)
