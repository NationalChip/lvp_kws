...
class GetBunKwsWordsBasicInfo:
    ...
    def get_keyword_info(self, keywords_filename):
        ...
        for single_line in data:
            ...
            if re.findall("是否主唤醒:", single_line):
                ...
                if str(is_main_wakeup) not in ["0", "1"]:
                    self.logger.log(f"[GetBunKwsWordsBasicInfo]: 错误！{keywords_filename} 文件中 {single_line} 是否为主唤醒设置错误，该值只能是0/1", "error")
                    fail_flag = 1
            ...
            if re.findall("事件id:", single_line):
                ...
                try:
                    ...
                    self.logger.log(f"[GetBunKwsWordsBasicInfo]: 错误！{keywords_filename} 文件中 {single_line} 事件ID 设置错误，该值只能是整数", "error")
                    fail_flag = 1
            ...
            if index in output_dict:
                self.logger.log(f"[GetBunKwsWordsBasicInfo]: 错误！{keywords_filename} 文件中有序索引 {index} 有重复", "error")
                fail_flag = 1
            if index == -1:
                continue
        ...
        self.logger.log(f"[GetBunKwsWordsBasicInfo]: 调试！{keywords_filename} 文件解析后的数据 {output_dict}", "debug")
        return output_dict
    ...
    def GetThresholdData(self):
        ...
        if not keywords_dict:
            self.logger.log(f"[GetBunKwsWordsBasicInfo]: 错误！未正确从 {keywords_filename} 文件中获取到有序关键字列表信息", "error")
            return {}
        ...
        for row in sheet.iter_rows(min_row=cmd_min_row, max_col=cmd_col, min_col=cmd_col, values_only=True):
            ...
            try:
                ...
                self.logger.log(f"[GetBunKwsWordsBasicInfo]: 错误！未正确从 {report_filename} 文件中获取到 {cmd} 的阈值信息", "error")
            threshold_row += 1
            if cmd in ["None", None]:
                continue
        ...
        for keywords_index in keywords_dict:
            ...
            if cmd not in cmd_threshold_dict:
                self.logger.log(f"[GetBunKwsWordsBasicInfo]: 错误！未正确从 {cmd} 关键字阈值信息未正确从 {report_filename} 中获取", "error")
                fail_flag = 1
            ...
            if "事件ID" in keywords_dict[keywords_index][cmd]:
                ...
                self.logger.log(f"[GetBunKwsWordsBasicInfo]: 提示！ 词: {cmd}  未在 {keywords_filename} 中配置 '事件ID'，将默认为 {event_id}", "info")

            is_main_wakeup = 1
            if "是否主唤醒" in keywords_dict[keywords_index][cmd]:
                is_main_wakeup = keywords_dict[keywords_index][cmd]["是否主唤醒"]
            else:
                self.logger.log(f"[GetBunKwsWordsBasicInfo]: 提示！ 词: {cmd}  未在 {keywords_filename} 中配置 '是否主唤醒'，将默认为主唤醒", "info")

            if not fail_flag:
                output_dict[keywords_index] = {"词": cmd, "阈值": cmd_threshold_dict[cmd],
                                                "事件ID": event_id, "是否主唤醒": is_main_wakeup,
                                                "词拼音": f'{" ".join(lazy_pinyin(cmd))}'}
        #print(output_dict)
        # 关闭工作簿（非必需，但建议）
        wb.close()
        if fail_flag:
