...
class RebulidAlchemyLvpAutoconf:
    ...
    def __init__(self, internal_msg_bus={}, logger=None):
        ...
        self.defconfig = os.path.join(self.internal_msg_bus["SDK 根路径"],
                                            "configs/kws_model_auto_deploy_config/alchemy_model_auto_deploy.config")
    ...
    def append_settings_to_dot_config(self, config_str):
        """
            功能:
                    * 在.config末尾追加配置
            参数:
            ...
            返回值:
                    * /
        """
        with open(self.output_config_path, 'a') as file:
    ...
    def generate_defconfig(self):
        ...
        try:
            # 清除可能影响的环境变量
            # 1. 移除LD_LIBRARY_PATH，避免使用临时解压目录中的库
            if 'LD_LIBRARY_PATH' in os.environ:
                del os.environ['LD_LIBRARY_PATH']
            # 2. 也可以考虑移除PyInstaller添加的其他环境变量，但主要的是LD_LIBRARY_PATH
            # 注意：如果系统原本有LD_LIBRARY_PATH，那么这里也会清除，但这正是我们需要的，因为希望用系统默认路径
            # 3. 但我们可能需要保留PATH，因为要找到make命令

            # 使用系统命令 make defconfig
            # 注意：这里使用os.system，因为它会使用当前设置的环境变量
            #ret = os.system("make defconfig")
            process, output, error = self.shell.Run("make defconfig")
            if process.returncode != 0:
                raise ValueError(f"\033[0;36;31m[RebulidOvpAutoconf]: 错误! 生成 autoconf.h 失败! 原因: {error}\033[0m")
            #if ret != 0:
            #    raise ValueError(f"\033[0;36;31m[RebulidOvpAutoconf]: 错误! 生成 autoconf.h 失败!\033[0m")
        finally:
            # 恢复环境变量
            os.environ.clear()
    ...
    def Start(self):
        ...
        self.append_settings_to_dot_config(dot_config)
        ...
        #3、写入 NPU_SIZE 信息
        npu_size = str(self.internal_msg_bus["模型关键信息"]["npu_size"]).strip()
        npu_size = math.ceil(int(npu_size)/1024)
        if int(npu_size) < 12:
            npu_size = 12
        dot_config = [[f"CONFIG_NPU_SRAM_SIZE_KB={str(npu_size)}\r\n"]]
        self.write_config_to_file(dot_config, self.output_config_path, "CONFIG_NPU_SRAM_SIZE_KB=150")

        #5、写 CONFIG_LVP_ENABLE_KEYWORD_RECOGNITION 信息
        dot_config = [[f"CONFIG_LVP_ENABLE_KEYWORD_RECOGNITION=y\r\n",
                       f"CONFIG_NORMAL_CTC_SCORE=y\r\n",
                       f"CONFIG_LVP_ENABLE_CTC_DECODER=y\r\n"
                    ]]
        self.write_config_to_file(dot_config, self.output_config_path, "# CONFIG_LVP_ENABLE_KEYWORD_RECOGNITION is not set")

        #6、写 CTC 版本
        dot_config = f"CONFIG_USE_CTC_VERSION_V0DOT0DOT3=y\r\n"
        self.append_settings_to_dot_config(dot_config)
        dot_config = f"CONFIG_LVP_MODEL_USE_RNN=y\r\n"
        self.append_settings_to_dot_config(dot_config)
        dot_config = "CONFIG_LVP_ENABLE_CTC_SCORE_COMPENSATOR=n\r\n"
        self.append_settings_to_dot_config(dot_config)
        dot_config = "CONFIG_LVP_ENABLE_OPTIMIZE_SHORT_INSTRUCTIONS=n\r\n"
        self.append_settings_to_dot_config(dot_config)
        dot_config = "CONFIG_LVP_ENABLE_CTC_BIONIC=n\r\n"
        self.append_settings_to_dot_config(dot_config)
        dot_config = "CONFIG_ENABLE_CTC_JUDGE_TO_REDUCE_FALSE_ACTIVATION=n\r\n"
        self.append_settings_to_dot_config(dot_config)
        dot_config = "CONFIG_VPA_ENABLE_ONLY_PRINTF_SCORE_WITHOUT_ACTIVATE=n\r\n"
        self.append_settings_to_dot_config(dot_config)
        dot_config = "CONFIG_VPA_ENABLE_DELAY_TO_DECODE=n\r\n"
        self.append_settings_to_dot_config(dot_config)

        #7、.config 生成  include/autoconf.h
