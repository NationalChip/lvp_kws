...
class CreateAlchemyAcousticModelPackageFile:
    """ 这是一个创建模型包相关文件的类 """
    def __init__(self):
    ...
    def create_kws_list_h(self, output_path, parsed_data_dict, internal_msg_bus):
        """
            ...
            返回值:
                    * /
        """
        kws_list_h_filename = output_path
        kws_list_h_content = f"""/* Grus
...
"""
        ...
        #print(word_max_len)
        for index in sorted(internal_msg_bus["词信息"].keys()):
            single_word_info = internal_msg_bus["词信息"][index]
            spaces_num = word_max_len + 1 - len(single_word_info["词"])
            kws_list_h_content += f'''#ifdef CONFIG_KEYWORD_ENABLE_{single_word_info['词拼音'].replace(' ', '_').upper()}
    {{"{single_word_info['词拼音']}", {{{", ".join(map(str, single_word_info["label"]))}}}, {single_word_info['label_length']}, CONFIG_KEYWORD_{single_word_info['词拼音'].replace(' ', '_').upper()}_THRESHOLD, CONFIG_KEYWORD_{single_word_info['词拼音'].replace(' ', '_').upper()}_VALUE, {single_word_info['是否主唤醒']}}},
#endif
'''
        kws_list_h_content += f"""}};

#endif /* __KWS_LIST_H__ */
"""
        with open(kws_list_h_filename, "w") as f:
            f.write(kws_list_h_content)
...
kws_objs       += $(MODEL_DIR_PRE)/ctc_model.o
    ...
    def create_kws_list(self, kws_list_filename, parsed_data_dict, internal_msg_bus):
        """
            功能:
                    * 生成 kws.list 文件
            参数:
        ...
        #2、使用版本
        merge_version = parsed_data_dict["acoustics model version"]
        kws_list_content = f"""menu "Kws List Settings"
depends on LVP_KWS_{project_name.upper()}_{merge_version.replace('.', 'DOT').upper()}_{datetime.now().strftime("%Y_%m%d")}
"""
        for index in sorted(internal_msg_bus["词信息"].keys()):
            single_word_info = internal_msg_bus["词信息"][index]
            kws_list_content += f"""
  config KEYWORD_ENABLE_{single_word_info['词拼音'].replace(' ', '_').upper()}
      bool "{single_word_info['词拼音']}"
      default y
      if KEYWORD_ENABLE_{single_word_info['词拼音'].replace(' ', '_').upper()}
          config KEYWORD_{single_word_info['词拼音'].replace(' ', '_').upper()}_THRESHOLD
              int "The Threshold of {single_word_info['词拼音']}"
              default {single_word_info['阈值']}

          config KEYWORD_{single_word_info['词拼音'].replace(' ', '_').upper()}_VALUE
              int "The Kws Value of {single_word_info['词拼音']}"
              default {single_word_info['事件ID']}

          comment ""
      endif
"""
        kws_list_content +=f"""endmenu
